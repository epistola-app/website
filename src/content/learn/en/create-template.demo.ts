...
await generate({
  ...
  auth: {
    ...
    storage: { name: "epistola-demo", types: ["cookies"] },
  },

  setup: [
    // Delete tenant if it exists (tolerant — won't fail)
    ...deleteTenantSteps,
    // Create fresh tenant for isolation
  ...
  scenes: [
    ...
    { narration: "We maken een nieuw template aan. Geef het een ID en een naam.", stepIndex: 4 },
    { narration: "Het template is aangemaakt. Je ziet de detailpagina met tabbladen voor varianten, deployments, datacontracten en instellingen.", stepIndex: 9 },
  ],
  ...
  steps: [
    ...
    // Scene 1: Dashboard → templates list
    ...
    // Scene 2: Templates list → New Template → fill form
    ...
    { action: "type", selector: { strategy: "id", value: "slug" }, text: "vergunning-brief", delayAfterMs: 300 },
    { action: "type", selector: { strategy: "id", value: "name" }, text: "Vergunning Brief", delayAfterMs: 500 },

    // Scene 3: Submit → template detail page
    ...
    { action: "waitFor", kind: "loadState", state: "networkidle" },
    { action: "wait", ms: 3000 },
  ],
}, { verbose: true });
